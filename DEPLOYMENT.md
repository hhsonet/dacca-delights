...
**Product codes and photos.** Every product has a fixed 6-character code
(letters and digits, with 0/O/1/I/L excluded so it survives being read aloud).
Codes are generated by the system and never accepted from a form.

Photos live on disk at `public/uploads/products/<CODE>/`, one folder per
product, indexed in `product_photos` so their order is stable. Six photos
maximum: the first is the primary image, and once the folder is full the next
upload overwrites the **last** slot rather than being rejected. Uploaded photos
take precedence over the seeded image URL on the storefront.

Each photo is marked **Real** or **AI** by staff on the product page, and that
mark follows the image everywhere it appears — product cards on the home and
menu pages, the product detail image and every thumbnail, search suggestions,
cart and checkout lines, order details, the admin product list and the admin
photo grid. Large images get a pill (`◉ Real` / `✦ AI`); thumbnails get a
corner dot with the same colour and tooltip. Replacing a slot resets its
marking, since it described the previous image.

Only images an admin has actually classified are marked. A product still using
a seeded stock URL shows **no badge at all** — labelling it "Real" would assert
something nobody verified.

Upload safety — this directory sits inside the web root, so it is locked down
on several levels:

- `public/uploads/.htaccess` turns the PHP engine off, denies script
  extensions, serves only image files and disables directory listing. Verified:
  a `.php` file placed in the tree returns **403** and does not execute.
- Every upload is **decoded and re-encoded** with gd, so bytes that were not
  part of the image do not survive. Verified against a real JPEG with PHP
  appended: the payload is absent from the stored file.
- The type comes from the decoded image, not the submitted filename; the
  filename on disk is generated; size is capped at 5 MB and dimensions at
  8000px per side.

`gd` must be enabled (`extension=gd` in `php.ini`) or uploads will fail —
re-encoding depends on it.
...
**Google sign-in.** "Continue with Google" on `/login` and `/signup` runs the
standard authorization-code flow (`league/oauth2-google`) via
`/auth/google` and `/auth/google/callback`.

To enable it, create an OAuth 2.0 Client ID (type: **Web application**) in the
Google Cloud console under *APIs & Services > Credentials*, add the callback as
an **Authorized redirect URI**, and put the values in `.env`:
...
The redirect URI must match what is registered **exactly** — scheme, host, port
and path. Until credentials are set the button explains it is not configured
rather than failing obscurely.

How accounts are matched, in order:

1. An existing `google_id` — the Google subject id, which is stable even if the
   person changes their email.
2. Otherwise the verified email, which **links** Google to an existing password
   account instead of creating a duplicate. The existing password keeps
   working; both routes sign in to the same account.
3. Otherwise a new customer is created, already email-verified.

Security notes: the `state` parameter is stored in the session and compared with
`hash_equals` on return, so a callback that did not originate from this site is
refused before any token exchange. An address Google reports as unverified is
rejected outright — accepting it would let someone claim an address they do not
own and take over that account. `google_id` is written outside the model's
allowed fields so it can never be mass-assigned from a form.
